  ...
  public Floor(int xCor, int yCor, int zone, boolean allowSpawn)
  {
    ...
    if (allowSpawn && LevelVar.SPAWN_MONSTERS)
    {
      if (LevelVar.rand.nextDouble() < 0.01 + (LevelVar.levelNum * LevelVar.spawnModifier))
      {
        ...
        {
          LevelVar.zombieCollection.add(new LineWalkZombie(LevelVar.rand.nextDouble() * 360, xCor + 0.5, yCor + 0.5, this, Level.nextZombie++, Level.nextZombie++, Level.nextZombie++));
        } else
        {
          LevelVar.zombieCollection.add(new RandomWalkZombie(LevelVar.rand.nextDouble() * 360, xCor + 0.5, yCor + 0.5, this, Level.nextZombie++, Level.nextZombie++, Level.nextZombie++));
        }
        isEmpty = false;
      }
    }
