...
//    skipAll = true;
  ...
  private void fillWithCoordoors(int zone)
  {
    ...
    for (int i = 0; i < zoneList[zone].zoneX; i++)
    {
      for (int j = 0; j < zoneList[zone].zoneY; j++)
      {
        ...
        {
          if (!zoneList[zone].subHouse[i][j].allWallsIntact())
          {
            continue;
          }
  ...
  private boolean attemptToPlace(int topX, int topY, int width, int height, Zone zone)
  {
    for (int i = 0; i < width; i++)
    {
      ...
      {
        if (zone.notInZone(topX + i, topY + j))
        {
          return false;
        }
        if (!zone.subHouse[topX + i][topY + j].allWallsIntact())
        {
          return false;
        }
      }
    ...
    for (int i = 0; i < width; i++)
    {
      for (int j = 0; j < height; j++)
      {
        if (i != width - 1)
        {
          zone.subHouse[topX + i][topY + j].breakWall(Direction.EAST);
        }
        if (i != 0)
        {
          zone.subHouse[topX + i][topY + j].breakWall(Direction.WEST);
        }
        if (j != height - 1)
        {
          zone.subHouse[topX + i][topY + j].breakWall(Direction.SOUTH);
        }
        if (j != 0)
        {
          zone.subHouse[topX + i][topY + j].breakWall(Direction.NORTH);
        }
      }
    }
    return true;
  ...
  private void findNeighbors()
  {
    for (int i = 1; i < fullX - 1; i++)
    {
      for (int j = 1; j < fullY - 1; j++)
      {
        ...
        {
          if (illegalIndex(i + dir.dX, j + dir.dY))
          {
            continue;
          }
  ...
  private void connectZones()
  {
    ...
    while (zonePairs < 3)
    {
      ...
      if (checkRow)
      {
        ...
        if (zoneList[tile1.zone].alreadyNeighbors(tile2.zone))
        {
          continue;
        }
  ...
  {
    if (!LevelVar.SPAWN_MONSTERS && !LevelVar.SPAWN_MASTER)
    {
      return;
    }
  ...
  private void placeExit(int zoneNum)
  {
    ...
    while (exitNotPlaced)
    {
      ...
      if (onVertWall)
      {
        ...
        if (!LevelVar.house[exitX + facing.dX][exitTop].isEmpty())
        {
          continue;
        }
      ...
      } else // on a horizontal wall
      {
        ...
        if (!LevelVar.house[exitLeft][exitY + facing.dY].isEmpty())
        {
          continue;
        }
  ...
  private class Room
  {
    ...
    private boolean alreadyNeighbors(int roomNum)
    {
      ...
      {
        if (roomNum == neighbor.roomNumber)
        {
          return true;
        }
      }
